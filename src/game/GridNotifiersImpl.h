...
MaNGOS::DelayedUnitRelocation::Visit(PlayerMapType &m)
{
    for(PlayerMapType::iterator iter = m.begin(); iter != m.end(); ++iter)
    {
        ...
        CellPair pair2(MaNGOS::ComputeCellPair(viewPoint->GetPositionX(), viewPoint->GetPositionY()));
        Cell cell2(pair2);
        //cell.SetNoCreate(); need load cells around viewPoint or player, that's why it commented

        PlayerRelocationNotifier relocate(*unit, *viewPoint);
        ...
        relocate.SendToSelf();
        unit->SetNotified(unit->GetNotifyFlags());
    }
}
